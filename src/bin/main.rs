...
fn main() -> ! {
    ...
    let mut spi = Spi::new(peripherals.SPI2, Config::default().with_frequency(esp_hal::time::Rate::from_mhz(80)).with_mode(Mode::_0))
        .expect("spi")
        .with_sck(sclk)
        .with_mosi(mosi);
    ...
    send_cmd(0xD6, &[0xA1]);
    send_cmd(0xE0, &[0xF0,0x06,0x0B,0x0A,0x09,0x26,0x29,0x33,0x41,0x18,0x16,0x15,0x29,0x2D]);
    send_cmd(0xE1, &[0xF0,0x04,0x08,0x08,0x07,0x03,0x28,0x32,0x40,0x3B,0x19,0x18,0x2A,0x2E]);
    ...
    let x0 = 0u16;
    let x1 = 240u16 - 1;
    send_cmd(0x2A, &[(x0 >> 8) as u8, x0 as u8, (x1 >> 8) as u8, x1 as u8]);
    send_cmd(0x2B, &[(y0 >> 8) as u8, y0 as u8, (y1 >> 8) as u8, y1 as u8]);
    ...
    loop {
        ...
        while offset < ANIM_DATA.len() {
            ...
            if offset + 4 > ANIM_DATA.len() { break; }
